...
typedef MyDraggableCanceledCallback = void Function(
...
class _MyDragTargetState<T> extends State<MyDragTarget<T>> {
  ...
  bool didEnter(DragAvatar<T> avatar) {
    assert(!_candidateAvatars.contains(avatar));
    assert(!_rejectedAvatars.contains(avatar));
    if ((widget.onWillAccept == null || widget.onWillAccept!(avatar.data))) {
      setState(() {
...
class DragAvatar<T> extends Drag {
  ...
  late _MyDragTargetState<T>? _activeTarget;
  ...
  Iterable<_MyDragTargetState<T>> _getMyDragTargets(
    ...
    for (HitTestEntry entry in path) {
      if (entry.target is RenderMetaData) {
        ...
        if (metadata is _MyDragTargetState<T>) yield metadata;
      }
    }
  }
